document.addEventListener("DOMContentLoaded", function () {
    ...
    productForm.addEventListener('submit', function (event) {
        ...
        if (imageFile) {
            ...
            reader.onload = function (event) {
                ...
                saveProduct(productName, price, productType.value, imageUrl); // Llamar a la función para guardar
            };
            reader.readAsDataURL(imageFile); // Leer la imagen como Data URL
        } else {
            saveProduct(productName, price, productType.value, imageUrl); // Manejar caso de no imagen
        }
    });

    function saveProduct(productName, price, type, imageUrl) {
        let productDetails = {
    ...
    function resetForm() {
        productForm.reset(); // Restablece el formulario a sus valores predeterminados
        ...
        // Restablece el tipo de producto a un valor inicial (opcional)
        productType.value = '';
    }

    // Modifica la parte donde abres el modal para limpiar el formulario
    addProductBtn.addEventListener('click', () => {
        productModal.style.display = 'block';
        resetForm(); // Llama a la función para limpiar el formulario
        editProductId = null; // no estar en modo edicion
        document.getElementById('form-title').textContent = 'Agregar Producto';
    });

    // También limpia el formulario cuando cerras el modal
    closeBtn.addEventListener('click', () => {
        productModal.style.display = 'none';
        resetForm(); // Llama a la función para limpiar el formulario
    });
